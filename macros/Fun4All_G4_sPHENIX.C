...
int Fun4All_G4_sPHENIX(
    const int nEvents = 1,
    const int nTracks = 1,
    const float zsigma = 10.,
    const float zsigma_pileup = 40.,
    const double pileup_collision_rate = 1.2e4, // 1.2e7,
    const char *inputFile = "hepmc_pythia/minbias/phpythia8_200gev_mb_00001.dat.gz",
    const char *pileupFile = "hepmc_pythia/minbias_pileup/phpythia8_200gev_mb_00001.dat.gz",
    std::string outputfile = "pileupeval.root",
    std::string outputfile2 = "multi_zvtx.root")
//    const char *inputFile = "/sphenix/data/data02/review_2017-08-02/single_particle/spacal2d/fieldmap/G4Hits_sPHENIX_e-_eta0_8GeV-0002.root",
    ...
    if (particles)
    {
      ...
      for (int i = 0; i<nTracks ; i++) {
        ...
        //gen->add_particles("pi-",2); // mu+,e+,proton,pi+,Upsilon
                  if (i%2 ==0) gen->add_particles("pi+",1);
        else gen->add_particles("pi-",1);
        ...
        char distribution[999];
        sprintf(distribution,"%f*pow(x,%f)*pow((1.+(x/%f)*(x/%f)),%f)*x",a0,a3,a1,a1,a2);
        TF1* ptdistr = new TF1("ptdistr",distribution,start,stop);
        pt_gen = ptdistr->GetRandom(); // non-flat pt used      

        gen->set_pt_range(pt_gen,pt_gen);
        } else {
        gen->set_pt_range(0.5, 10.5);
        //gen->set_pt_range(10. - 0.2*i, 10.0 - 0.2*i);
                  }
      gen->Embed(1);
      gen->Verbosity(0);
  ...
  if (do_tracking_track) Tracking_Reco(0,outputfile2.c_str());
  ...
  if (do_tracking_eval) Tracking_Eval(outputfile.c_str());
  ...
  if (readhepmc)
  {
    ...
    PHHepMCGenHelper::VTXFUNC uniform  = PHHepMCGenHelper::Uniform;
    PHHepMCGenHelper::VTXFUNC gaus   = PHHepMCGenHelper::Gaus;
    in->set_vertex_distribution_function(gaus, gaus, uniform, gaus);
    in->set_vertex_distribution_width(100e-4.,100e-4,zsigma,0);//optional collision smear in space time
...
//   const string pileupfile("/gpfs/mnt/gpfs04/sphenix/user/shlim/04.InnerTrackerTaskForce/01.PythiaGen/HepMC_files/phpythia8_SoftQCD_inelastic_200GeV_hepmc_0100_run000000.dat");
//    pileup->AddFile(pileupfile);
    pileup->Verbosity(2);
    pileup->set_vertex_distribution_width(100e-4,100e-4,zsigma_pileup,5);//override collision smear in space time
