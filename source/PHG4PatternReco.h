...
public:
	...
	void set_ladder_type(const int* ladderType, const int n) {
		_ladder_type.assign(ladderType, ladderType + n);
	}
...
private:
	...
	int cellular_automaton_mvtx(std::vector<Track3D>& candidate_tracks);
	int cellular_automaton_intt(std::vector<Track3D>& candidate_tracks);
	int fit_vertex();	
	...
	float _ca_phi_cut;
	float _ca_z_cut;
	float _ca_zproj_phi_cut;
	float _ca_zproj_z_cut;
	float _ca_pproj_phi_cut;
	float _ca_pproj_z_cut;
	float _z_cut;
	float _mult_onebin;
	float _mult_twobins;
