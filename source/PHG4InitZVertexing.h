...
///
class PHG4InitZVertexing: public SubsysReco {

public:

	PHG4InitZVertexing(unsigned int nlayers = 7, unsigned int min_nlayers = 7,
			const std::string &name = "PHG4InitZVertexing");
	virtual ~PHG4InitZVertexing() {
	}
...
//	unsigned int _triplet_lbegin; // starting layer for triplet building in forward mode
//	unsigned int _triplet_lendl; // starting layer for triplet building in backward mode
	// layer range for cellular automaton, lbegin is not necessarily first of seeding layers
	unsigned int _ca_lbegin;
	unsigned int _ca_lend;
