/*!
 *  \file               PHG4InitZVertexing.C
 *  \brief              Initial Z-vertex finding algorithm using MAPS detectors
 *  \date               6/4/2018
...
#define _MULTIVTX_ // to fill multivtx histograms
#define _TRIPLETS_
...
//        _ca_nlayers = 3; // 3(0,1,2 for vertexing)/4(0,1,2 3 for track seeding) for p+p
...
int PHG4InitZVertexing::create_nodes(PHCompositeNode* topNode) {
	...
	PHIODataNode<PHObject>* tracks_node = new PHIODataNode<PHObject>(_trackmap,
			"SvtxTripletMap", "PHObject");
	tb_node->addNode(tracks_node);
	if (Verbosity() > 0)
		cout << "Svtx/SvtxTripletMap node added" << endl;
...
int PHG4InitZVertexing::get_nodes(PHCompositeNode* topNode) {
	...
	// Pull the reconstructed track information off the node tree...
	_trackmap = findNode::getClass<SvtxTrackMap>(topNode, "SvtxTripletMap");
	if (!_trackmap) {
...
int PHG4InitZVertexing::export_output(){
	...
	_trackmap->clear();
        if (_tracks.empty())
                return Fun4AllReturnCodes::EVENT_OK;
        unsigned int tracks_size=_tracks.size();
	std::vector<SvtxVertex_v1> svtx_vertex_list;
	unsigned int nvertex = _vertex_list.size();
	//for TEST only : if there is only 1 track, save a fake vertex to save that track
	if(tracks_size==1)nvertex=1; 
	...
	//	vertex.set_id(vid);
        ...
        // vt is assigned -9 by default until in-time vertex is identified   
        	vertex.set_t0(-9.0);
        	for (int i = 0; i < 3; ++i){
			if(tracks_size==1){vertex.set_position(i,0.);}
			else{ vertex.set_position(i, _vertex_list[vid][i]);}
		}
	...
	track_hits.clear();
        int clusterID;
        for (unsigned int itrack = 0; itrack < tracks_size; ++itrack) {
		if (_tracks.at(itrack).vertex_id >= nvertex && tracks_size!=1 ) continue;
                SvtxTrack_v1 track;
                track.set_id(itrack);
                track_hits.clear();
...
int PHG4InitZVertexing::build_triplets_to_Track3D(std::vector<Track3D>& new_tracks, bool forward){
	...
	// first 3 layers if forward
	unsigned int layer0 = _ca_lbegin+0;
	unsigned int layer1 = _ca_lbegin+1;
	unsigned int layer2 = _ca_lbegin+2;
	if (!forward) {
		layer0 = _ca_lend-_ca_lbegin;
		layer1 = _ca_lend-_ca_lbegin-1;
		layer2 = _ca_lend-_ca_lbegin-2;
	}
...
//		cout<<"layer "<< layer<<" hitid "<<hitid<<endl;
	...
	cout<<"layer 2 " <<layer_sorted_2.size()<<endl;
        ...
        cout<<"ca_phi_cut "<<_ca_phi_cut<<" z_cut "<<_z_cut<<endl;
...
//		cout<<"cluster_id for hit 0 "<< *it0<<endl;
		...
		for (std::vector<unsigned int>::iterator it1 = layer_sorted_1.begin(); it1 != layer_sorted_1.end(); ++it1 )
		{
			...
			//cout<<"phi h0 "<<phi_h0<<" phi_h1 "<<phi_h1<<" phi_h0h1 "<<phi_h0h1<<endl;
			for(std::vector<unsigned int>::iterator it2 = layer_sorted_2.begin(); it2 != layer_sorted_2.end(); ++it2)
			{
				...
				float z_h2 = hit2.get_z();
                        //cout<<"phi h0 "<<phi_h0<<" phi_h1 "<<phi_h1<<" phi_h2 "<<" phi_h0h1 "<<phi_h0h1<<" phi_h1h2 "<<phi_h1h2<<endl;
			//cout<<"z_h0 "<<z_h0<<" z_h1 "<<z_h1<<" z_h2 " <<z_h2<<endl;
...
//	ca->set_propagate_forward(false);// need to implement triplet in backward propagation
	ca->set_propagate_forward(true);
        ...
        ca->set_process_mode(1); // HT triplet ProjZ
...
int PHG4InitZVertexing::fit_vertex(){
	if (Verbosity()>0)    { cout<<"tracks size " << _tracks.size()<<endl;}
	if (_tracks.size()==1){ return 0;}
	...
	unsigned int nzvtx = vtx_tracks.size();
        ...
        // range (-32, 32) 320000/1280 = 250um to acheive resolution better than 500 um
        unsigned int nzbins= 10000*_max_z0/250;
	nzbins *= 2;
	float binsize = (_max_z0-_min_z0)/nzbins;
        float zvalues[nzbins];
        unsigned int zcounts[nzbins];
	...
	std::vector<float> zmeans; // average z for tracks within 500um window
...
//						/(zcounts[j-1]+zcounts[j]+zcounts[j+1]);
	...
	if(Verbosity())
	{
		...
		{
		cout<<"candidate primary vertex position : "<<zvertices[j]<<endl;
		}
	}
