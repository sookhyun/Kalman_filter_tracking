/*!
 *  \file               PHG4PatternReco.C
 *  \brief              In-time vertex finding and track seeding algorithms using MAPS + INTT detectors
 *  \date               6/4/2018
...
#define _TRIPLETS_ // use SvtxTrack's directly without reconstructing more triplets, if not defined 
...
PHG4PatternReco::PHG4PatternReco(unsigned int nsublayers,
    ...
    : SubsysReco(name),
      ...
      _nlayers((nsublayers-3)/2+3),
      _min_nlayers(min_nlayers),
      _ca_lbegin(0),
      _ca_lend(3),
      _ca_min_nlayers(_ca_lend - _ca_lbegin +1),
      _ca_chi2(2.),
      _radii(),
      _material(),
      _user_material(),
      ...
      _ca_phi_cut(M_PI/36.),
      _ca_z_cut(20.),
      _ca_zproj_phi_cut(M_PI/110),
      _ca_zproj_z_cut(1.),
      _ca_pproj_phi_cut(M_PI/16),//= +-0.20 /+-0.13
      _ca_pproj_z_cut(0.05),
      _z_cut(1.),
      _mult_onebin(1.),
      _mult_twobins(1.),
      _mult_threebins(1.),
...
int PHG4PatternReco::InitRun(PHCompositeNode* topNode) {
    ...
    #ifdef _FILLHISTO1_
    _h_intt_zres = new TH1D("h_intt_zres","h_intt_zres",200,-2.,2.);
    #endif 
#endif
	...
	if (Verbosity() > 0) {
		...
		cout << " Number of tracking layers: " << _nsublayers << endl;
		for (unsigned int i = 0; i < _nsublayers; ++i) {
...
//        _ca_nlayers = 4; // 3(0,1,2 for vertexing)/4(0,1,2,3 for track seeding) for p+p
...
#ifdef _TRIPLETS_  // turnoff_hits_used not working properly, resulting in redundant tracks
                        _temp_tracks.clear();
                        build_triplets_to_Track3D(_temp_tracks, true); // false : backward

			code = 0;
			code = cellular_automaton_mvtx(_temp_tracks); // remove bad hits, save fit parameters && turn off used hits
			if (code != Fun4AllReturnCodes::EVENT_OK)
			{
			cout<<"CellularAutomaton_MVTX failed. "<<endl;
			exit(1);
			}
#endif

			// To do : takein _temp_tracks from ca_mvtx and extend them in ca_intt
			// ca will have to have another function that takes in triplets with all parameters comupted 
			// and use them to project in z for INTT    
			// keep the extented part for TPC where you need more than 3 hits.
			_temp_tracks = _tracks;
			_tracks.clear();
			_track_states.clear();
			code = 0;
			code = cellular_automaton_intt(_temp_tracks);//_temp_tracks: additional tracks
			if (code != Fun4AllReturnCodes::EVENT_OK)
			{
			cout<<"CellularAutomaton_INTT"<<endl;
			exit(1);
			}	
		}// iattempt
...
int PHG4PatternReco::initialize_geometry(PHCompositeNode *topNode) {
	...
	_nsublayers = _seeding_layer.size();


	_radii.assign(_nsublayers, 0.0);
...
int PHG4PatternReco::get_nodes(PHCompositeNode* topNode) {
	...
	if (!_trackmap) {
        ...
        _tripletmap = findNode::getClass<SvtxTrackMap>(topNode, "SvtxTripletMap");
        if (!_tripletmap) {
                cerr << PHWHERE << " ERROR: Can't find SvtxTripletMap." << endl;
                return Fun4AllReturnCodes::ABORTEVENT;
        }
                ...
                iter != _clustermap->end(); ++iter) 
	{
        	...
        	SvtxCluster* cluster = iter->second;
                ...
                if(it != _layer_ilayer_map.end())
                        ilayer = it->second;
                if(ilayer >= _nsublayers) continue;
...
int PHG4PatternReco::export_output(){
	...
	// store vertex object in a map (so that it can have an vertex_id < 0)	
	// and assign a vertex_id to each track
	std::map<int, SvtxVertex*> svtx_vertex_list;
	std::map<int, int> svtx_vertex_ntriplets;
        std::map<int, int> svtx_vertex_ntracks;   
	// store all vertexes on node in a map
	...
	++iter) {
		...
		svtx_vertex_ntracks[vertex_id] = 0;
		svtx_vertex_ntriplets[vertex_id] = (int) vertex->size_tracks();
	}
	// add a vertex with a position = 999 for triplets not tied to a reco vertex, 
	// vertex_id is automatically assigned a positive integer
	// vt is assigned -9 by default until in-time vertex is identified   
	SvtxVertex_v1 fake_vertex;
        fake_vertex.set_t0(-9.0);
        for (int i = 0; i < 3; ++i)
	...
	track_hits.clear();
	unsigned int tripletmapsize = _tripletmap->size();
        int clusterID;
	cout<<"total number of tracks "<<_tracks.size()<< " tripletmap size "<< tripletmapsize<<endl;
        for (unsigned int itrack = 0; itrack < _tracks.size(); ++itrack) 
	{
		//if a track has an track id, find it in SvtxTrackMap and update it(substitute a new SvtxTrack)
		//or else insert a new SvtxTrack
//		unsigned int track3did =_tracks.at(itrack).get_id();
//		unsigned int trackid = (track3did>=0 && track3did<tripletmapsize) ? track3did : tripletmapsize+itrack;
                SvtxTrack_v1 track;
		track.set_id(itrack);
		// track_id is automatically assigned upon insertion, no need to set
                track_hits.clear();
                track_hits = _tracks.at(itrack).hits;

		if (track_hits.size()==0) {
			if(itrack >= tripletmapsize) continue;
			SvtxTrack* triplet = _tripletmap->find(itrack)->second;
			if (Verbosity()==3) cout<<"triplet id "<<triplet->get_id()<<endl;
			_trackmap->insert(triplet);
			continue;
		}
		if (Verbosity()==3) cout<<"-----------------------------------------------------"<<endl;
		if (Verbosity()==3) cout<<"track id "<<itrack<<" hits size "<<track_hits.size() <<endl;
//		cout<<"hits vector assigned " <<endl;
		...
		//if (Verbosity() ==3) cout<<"hits added to a track "<<endl;
		...
		//if(Verbosity() ==3) cout<<"computing pz"<<endl;
                float pZ = 0;
                if (/*dzdl != 1*/ dzdl<1.) {
...
//                track.set_chisq(_track_errors[itrack]);
		track.set_chisq(_track_states[itrack].chi2);
                track.set_ndf(ndf);
...
//                track.set_dca2d_error(sqrt(_track_covars[itrack](1, 1)));
		...
		//if (Verbosity()==3) cout<<"track parameters set "<<endl;
                if (_mag_field > 0) {
...
*/
  ...
  //                      track.set_error(row, col, euclidean_cov(row, col));
		...
		if (fabs(distance)>0.05) vid = 9999;		
		if (Verbosity() ==3 ) cout<<"vertex_id "<<vid<<" z0 "<<z0<<" distance "<<distance<<endl;
		// pca 
		if (vid==9999){
		track.set_x(0.);
		track.set_y(0.);
		...
		++svtx_vertex_ntracks[vid];
		if (Verbosity() ==3) cout<<"vertex_id "<< vid <<" incremented "<<endl;
		}

                unsigned int trackid = track.get_id();
                if (Verbosity() ==3) cout<<"adding track id "<< trackid<<endl;

			_trackmap->insert(&track);
		//if (Verbosity() ==3) cout<<"adding track id"<< trackid <<"to associated vertex "<<endl;
		if (vid==9999) fake_vertex.insert_track(track.get_id());
	// To do: add new tracks with legit vid to vertex 
	...
	if (Verbosity() >=3) cout<<"setting t0 of intime vertex to zero"<<endl;
/*
	unsigned int intime_vtx = 999;
	int max_tracks = 0;
	for(std::map<int,int>::iterator it= svtx_vertex_ntracks.begin(); it!=svtx_vertex_ntracks.end();++it)
	{
		if (it->second > max_tracks){
		max_tracks=it->second;
		intime_vtx = it->first;
		}
	}

	if (intime_vtx!=999) _vertexmap->get(intime_vtx)->set_t0(0.0);
*/
	unsigned int intime_vtx=999;
	int min_tracks = 1;
        for(std::map<int,int>::iterator it= svtx_vertex_ntracks.begin(); it!=svtx_vertex_ntracks.end();++it)
        {
		int ntriplets = svtx_vertex_ntriplets[it->first];
		if (ntriplets<=4){ min_tracks= floor(0.5*ntriplets);
		}else if(ntriplets>4 && ntriplets<10){min_tracks=floor(0.4*ntriplets);
		}else if (ntriplets>=10){min_tracks=floor(0.3*ntriplets);}
//	cout<<"ntriplets "<< ntriplets<<" min tracks "<<min_tracks<<" intime tracks "<<it->second<<endl;
                if (it->second >= min_tracks){
                	intime_vtx = it->first;
			if (intime_vtx!=999) _vertexmap->get(intime_vtx)->set_t0(0.0);
 			// This is to see the rario of nintimetracks to ntracks for oot vretex 
 			//   identified as intime and find an optimum cut to reject oot vertex. 
			//if (intime_vtx!=999) _vertexmap->get(intime_vtx)->set_t0(it->second);
                }
        }
	
	if (Verbosity() >=3) cout<<"adding fake vertex (for non-associated tracks)"<<endl;
...
int PHG4PatternReco::turnoff_hits_used_in_triplets(){
        ...
        for (SvtxTrackMap::Iter iter = _tripletmap->begin();
        iter != _tripletmap->end();
        ++iter) {
...
// Find more triplets after finding vertices in PHG4InitZVertexing
int PHG4PatternReco::cellular_automaton_mvtx(std::vector<Track3D>& candidate_tracks){

	_ca_lbegin = 0; _ca_lend = 3; 
	// get additional triplets using residual hits on 3 mvtx layers and 1st intt layer
	cout<<"Entering cellular autumaton mvtx : processing "<< candidate_tracks.size()<<" tracks. "<<endl;
...
//	ca->set_propagate_forward(false);// need to implement triplet in backward propagation
	ca->set_propagate_forward(true);
        ca->set_process_mode(1);     // 0: HT, 1: tracklet (continuous triplet/quadraplet/multiplet) 2: extended triplet 
	ca->set_seeding_mode(false); // false : normal mode, true: cheating mode ; will be obsolete if z-projection method works
				     // also place holder for triplet (as apposed to tracklet) mode	
//	ca->set_ladder_type(_ladder_type);
        ca->set_hits_map(hits_map);

	ca->set_remove_inner_hits(true);
//	ca->set_n_layers(_ca_nlayers);
	ca->set_layer_begin(_ca_lbegin);
	ca->set_layer_end(_ca_lend);
	ca->set_required_layers(_ca_min_nlayers);
	ca->set_ca_chi2_layer(2.);// 2.0
	ca->set_ca_chi2(_ca_chi2*10);
...
int PHG4PatternReco::cellular_automaton_intt(std::vector<Track3D>& additional_tracks){

	// translate SvtxTrack to Track3D
	std::vector<Track3D> candidate_tracks;
        for (SvtxTrackMap::Iter iter = _tripletmap->begin();
        iter != _tripletmap->end();
        ++iter) {
		Track3D triplet;
		triplet.hits.assign(3,Cluster3D());
                SvtxTrack* track = iter->second;
		triplet.set_id(track->get_id());
//		cout<<"ca_intt:: track id "<<triplet.get_id()<<endl;	
                for (SvtxTrack::ConstClusterIter iter = track->begin_clusters();
                iter != track->end_clusters();
                ++iter) {
                unsigned int cluster_id = *iter;
                	auto search = hits_map.find(cluster_id);
                	if (search != hits_map.end()){
                	Cluster3D cluster = search->second;
			triplet.hits[cluster.get_layer()] = cluster;
			}
                }
	candidate_tracks.push_back(triplet);	
        }
	
	// add additional tracks to candidate_tracks if TRIPLET mode is on
	unsigned int ntriplets = candidate_tracks.size();
	for(unsigned int it=0; it<additional_tracks.size();++it){
	Track3D triplet = additional_tracks[it];
	triplet.set_id(ntriplets+it);
	candidate_tracks.push_back(triplet);
	}

	// INTT simulation test
/*	   
	std::vector<std::vector<Cluster3D> > layer_sorted;
        std::vector<Cluster3D> one_layer;
        layer_sorted.clear();
        layer_sorted.assign(5, one_layer);

        for (std::map<unsigned int,Cluster3D>::iterator jt = hits_map.begin();
                        jt!= hits_map.end();
                        ++jt) {
	...
	//end test
*/
	// put triplets through ca and get silicon tracklets
	_ca_lbegin = 3; _ca_lend = 3;
	cout<< "Entering cellular automaton intt : processing "<<candidate_tracks.size()<<" tracks "<<endl;
        ca =    new CellularAutomaton_v1(candidate_tracks,_radii,_material);
	ca->set_verbosity(1);// 3: neat detailed print out
        ca->set_hough_space(_hough_space);
        ca->set_mag_field(_mag_field);
        ca->set_pt_rescale(_pt_rescale);
        ca->set_remove_hits(true);
        ca->set_propagate_forward(true);
        ca->set_process_mode(2); // 0: HT, 1: tracklet (continuous triplet/quadraplet/multiplet) 2: extended triplet
        ca->set_seeding_mode(false); // false : normal mode, true: cheating mode ; will be obsolete if z-projection method works
                                     // also place holder for triplet (as apposed to tracklet) mode
	ca->set_ladder_type(_ladder_type);
        ca->set_hits_map(hits_map);
        ...
        ca->set_layer_end(_ca_lend);
        ca->set_required_layers(_ca_min_nlayers);
        ca->set_ca_chi2_layer(1.);// 1.0
        ca->set_ca_chi2(_ca_chi2);
        ca->set_ca_zproj_phi_cut(_ca_zproj_phi_cut);
        ca->set_ca_zproj_z_cut(_ca_zproj_z_cut);
	ca->set_ca_pproj_phi_cut(_ca_pproj_phi_cut);
	ca->set_ca_pproj_z_cut(_ca_pproj_z_cut);
        ca->set_ca_dcaxy_cut(_dcaxy_cut);
        int code = ca->run(_tracks, _track_states, hits_used); // push_back output tracks into _tracks
	float var = ca->get_var();
	cout<<var<<endl;
	ca->Reset();
	#ifdef _FILLHISTO1_
	_h_intt_zres->Fill(var);
	#endif
        for(unsigned int i=0; i<candidate_tracks.size(); ++i) candidate_tracks[i].reset();
        candidate_tracks.clear();
	if (!code) {return 0;}
